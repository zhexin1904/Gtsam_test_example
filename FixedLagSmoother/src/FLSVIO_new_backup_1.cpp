...
gtsam::NonlinearFactorGraph addLandmarkToGraph(int kfID,
  ...
  for (const LandmarkId& lmk_id : currentLmk_id_) {
    ...
    // ? if SW does not contain this kf, firstKFID_it will be kfID_it
...
int main(int argc, char* argv[]) {
    ...
    std::string in_frontend("/home/jason/DPGO/FixedLagSmoother/5_1_easy_Lmk.txt");
    std::string in_initials("/home/jason/DPGO/FixedLagSmoother/5_1_easy_initials.txt");
...
/*                                     std::cout << "before linearizing :" <<std::endl;
    ...
    average_update = std::accumulate(T.begin(), T.end(), average_update) / T.size();
    std::cout << "average update time : " << average_update << std::endl;
    return 0;
}
