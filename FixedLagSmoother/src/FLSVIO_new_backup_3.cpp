// original version with landmark check of 9/7/2024
...
gtsam::FactorIndices intersect(const gtsam::FactorIndices& vec1, const gtsam::FactorIndices& vec2) {
    gtsam::FactorIndices sortedVec1 = vec1;
    gtsam::FactorIndices sortedVec2 = vec2;

    std::sort(sortedVec1.begin(), sortedVec1.end());
    std::sort(sortedVec2.begin(), sortedVec2.end());

    gtsam::FactorIndices intersection;
...
gtsam::FactorIndices checkLandmark(SmartFactorMap& smartFactorMap_,
                   std::unique_ptr<gtsam::BatchFixedLagSmoother>&& smootherBatch_) {
    gtsam::FactorIndices toDelete_;        

    const gtsam::NonlinearFactorGraph& new_graph = smootherBatch_->getFactors();
                    int ind_valid_check = 0;
                    for (const auto& factor : new_graph) {
                            const auto gsf = dynamic_cast<const RigFactor*>(factor.get());
                            if (gsf) {
                                const gtsam::TriangulationResult& result = gsf->point();
                                if (!result.valid()) {
                                    toDelete_.push_back(ind_valid_check);
                                    for (auto it = smartFactorMap_.begin(); it != smartFactorMap_.end(); ++it) {
                                        if (it->second.second == ind_valid_check) {
                                            smartFactorMap_.erase(it);
                                            break;
                                        }
                                    }                                  
                                }
...
//
    ...
    gtsam::FactorIndices toDelete_invalidLmk;
    ...
    //              get all the feature tracks for the current frame
                ...
                // SW.addElement(frame);
                    ...
                    try {
                        ...
                        smootherBatch->update(new_graph, initialEstimate, PoseTimestamps, intersect(toBeDeleted, toDelete_invalidLmk));
                        ...
                        toDelete_invalidLmk.clear();
                        toDelete_invalidLmk = checkLandmark(globalSmartFactorMap, std::move(smootherBatch));
...
//                            get the hessian factor for the linear gaussian factor
                    ...
                    std::cout << "Smoother recovery failed." << std::endl;
                    return false;
                    }
