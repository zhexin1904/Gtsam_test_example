...
struct IMUHelper {
  IMUHelper() {
    ...
    biasNoiseModel = gtsam::noiseModel::Isotropic::Sigma(6, 1e-3);
    velocityNoiseModel = gtsam::noiseModel::Isotropic::Sigma(3, 0.1);

    boost::shared_ptr<gtsam::PreintegratedCombinedMeasurements::Params> imuParams;
    imuParams = gtsam::PreintegratedCombinedMeasurements::Params::MakeSharedU(-9.8);
    // acc_noise
    imuParams->setAccelerometerCovariance(gtsam::I_3x3 * pow(0.001119, 2.0));
    // gyr_noise
    imuParams->setGyroscopeCovariance = gtsam::I_3x3 * pow(4.8e-05, 2.0);  
    // acc_walk
    imuParams->biasAccCovariance = gtsam::I_3x3 * pow(0.000167, 2.0);  
    // gyr_walk
    imuParams->biasOmegaCovariance = gtsam::I_3x3 * pow(3.0e-06, 2.0);  
    params->omegaCoriolis = w_coriolis;
    // integartion uncertainty
    double integration_sigma = 1e-8;
    imuParams->setIntegrationCovariance( gtsam::I_3x3 * pow(integration_sigma, 2) );

    imuParams->biasAccOmegaInt = gtsam::I_6x6 * 1e2;// 1e-5;
    // 
    imuParams->body_P_sensor = Pose3::identity();

    gtsam::Rot3 prior_rotation = Rot3(I_3x3);
    gtsam::Pose3 prior_pose(prior_rotation, Point3(0, 0, 0));

    gtsam::Vector3 acc_bias(0.0, -0.0942015, 0.0);  // 
    gtsam::Vector3 gyro_bias(-0.00527483, -0.00757152, -0.00469968);
    // get the PriorImuBias from online estimation
    //
    priorImuBias = imuBias::ConstantBias(gtsam::Vector3(-0.009684, -0.000818, -0.000818), 
                                        gtsam::Vector3(-0.131257, 0.160433, -9.761891));
    ...
    preintegrated = new PreintegratedCombinedMeasurements(imuParams, priorImuBias);
  }

  gtsam::imuBias::ConstantBias priorImuBias;  
  gtsam::noiseModel::Isotropic::shared_ptr velocityNoiseModel;
  gtsam::noiseModel::Isotropic::shared_ptr biasNoiseModel;
...
//
    ...
    std::streampos lastPositionImu = in_readImu.tellg();
    int currentKfID;
    bool firstLine = true;
...
//            pose.print();
    ...
    //           get the roll pitch and yaw from body_pose and x, y, z from body_pose
            ...
            while () {
                char line_imu[1024];
                in_readImu.getline(line_imu, sizeof(line_imu))
                stringstream ss_readImu(line_imu);
                ss_readImu >> frameIMu;
                if (firstLine) {
                    currentKfID = frameIMu;
                    firstLine = false;
                }
                if (notFirstFrame == true) {

                    if (frameIMu == frame - 1) {
                        double ax, ay, az;
                        double gx, gy, gz;
                        double dt = 1 / 187.28;  // IMU at ~800Hz
                ...
                gtsam::CombinedImuFactor imuFactor(X(frame - 1), V(frame - 1),
                            X(frame), V(frame), B(frame - 1),
                            B(frame), *imu.preintegrated);

                otherFactorGraph.push_back(imuFactor);
            ...
            if (!initialized) {
                ...
                priorFactorGraph.emplace_shared<gtsam::PriorFactor<gtsam::Vector3<(V(frame), Vector3(0, 0, 0), imu.velocityNoiseModel);
                initialEstimate.insert(V(frame), Vector3(0, 0, 0));
                PoseTimestamps[V(frame)] = count;

                graph.addPrior(B(frame), imu.priorImuBias, imu.biasNoiseModel);
                // get bias prior from online estimation
                initialEstimate.insert(B(frame), imuBias::ConstantBias(Vector3(-0.009684, -0.000818, -0.000818), Vector3(-0.131257, 0.160433, -9.761891)));
                PoseTimestamps[B(frame)] = count;
                ...
                gtsam::NavState propState = imu.preintegrated->predict(imu.prevState, imu.prevBias);
                initialEstimate.insert(V(frame), propState.v());
                PoseTimestamps[V(frame)] = frame;

                initialEstimate.insert(B(frame), imu.prevBias);
                PoseTimestamps[B(frame)] = frame;
    ...
    //              get all the feature tracks for the current frame
                ...
                if (!new_graph.empty()) {
                    new_graph.push_back(otherFactorGraph);
                    otherFactorGraph.resize(0);
                }
    ...
    //                            get the hessian factor for the linear gaussian factor
                                ...
                                if (!(success || maxrank == hessian_factor.rows() - 1)) {
                                    ...
                                    ++delete_count;
                                    // std::cout << "before linearizing :" <<std::endl;
                                    // if (ng.exists(ind)) {
                                    //     ng.at(ind)->print();
                                    // }
                ...
                imu.prevState = NavState(estimate.at<gtsam::Pose3>(X(frame)),
                                        estimate.at<gtsam::Vector3>(V(frame)));
                imu.prevBias = estimate.at<gtsam::imuBias::ConstantBias>(B(frame));
    ...
    for (auto pose : traj) {
        ...
        pose.second(3, 0) << " " << pose.second(3, 1) << " " << pose.second(3, 2) << " " << pose.second(3, 3) << "\n";
    }
    os_traj.close();
    return 0;
}
